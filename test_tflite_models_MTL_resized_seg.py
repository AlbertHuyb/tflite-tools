...
text_output_dir = "./new_profile_text_MTL_resized_seg"
if not os.path.exists(text_output_dir):
    os.makedirs(text_output_dir)


plot_output_dir = "./new_profile_plot_MTL_resized_seg"
...
for ratio in [0.1]:
    ...
    for task in ['cls','seg','det','pose']:
        ...
        if task == 'seg':
            tflite_path = os.path.join(checkpoint_dir, "mobilenetv2_resized_%s_pretrain_%.1f.tflite"%(task,ratio))
        ...
        tflite_output_path = os.path.join(checkpoint_dir, "optimized_mobilenetv2_%s_pretrain_%.1f.tflite"%(task,ratio))
